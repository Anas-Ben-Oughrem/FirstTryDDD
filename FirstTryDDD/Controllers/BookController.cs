...
namespace FirstTryDDD.API.Controllers
{
    ...
    public class BookController : ParentController
    {
        ...
        #region Post
        [HttpPost]
        public async Task<IActionResult> Post(PostBookRequest model)
        {
            try
            {
                if (model == null)
        ...
        #region Put
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, PutBookRequest model)
        {
            try
            {
                if (id == default)
